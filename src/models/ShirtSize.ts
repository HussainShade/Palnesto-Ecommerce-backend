...
import type { IShirt, IDiscount } from './Shirt.js';
import type { ISizeReference } from './SizeReference.js';

/**
 * ShirtSize document interface
 * Represents a specific size variant of a shirt
 * Each shirt can have multiple size variants with different prices and images
 */
export interface IShirtSize extends Document {
  shirtId: mongoose.Types.ObjectId | IShirt; // Foreign key to Shirt
  sizeReferenceId: mongoose.Types.ObjectId | ISizeReference; // Foreign key to SizeReference (M, L, XL, etc.)
  price: number; // Price for this specific size (varies by size)
  imageURL?: string; // Image URL for this specific size
  stock: number;
  finalPrice: number; // Computed field: price with discount applied from parent Shirt
  createdAt: Date;
  updatedAt: Date;
}
 ...
 */
const shirtSizeSchema = new Schema<IShirtSize>(
  {
    shirtId: {
...
shirtSizeSchema.pre('save', async function () {
  if (this.price !== undefined && this.shirtId) {
    ...
    if (shirt && shirt.discount) {
      const discount: IDiscount = shirt.discount;
...
export const ShirtSize = mongoose.model<IShirtSize>('ShirtSize', shirtSizeSchema);
