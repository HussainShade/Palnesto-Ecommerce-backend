 ...
 */
const sizeReferenceCache = new Map<string, string>(); // name -> ObjectId
const shirtTypeCache = new Map<string, string>(); // name -> ObjectId

/**
 * Initializes the reference data cache
 * Should be called on application startup
 */
export const initializeReferenceCache = async (): Promise<void> => {
  try {
    // Load SizeReferences
 ...
 */
export const refreshReferenceCache = async (): Promise<void> => {
 ...
 */
export const resolveSizeReference = (sizeParam: string): string => {
  ...
  if (!sizeId) {
    ...
    throw new Error(
      `Invalid size: '${sizeParam}'. Valid sizes are: ${validSizes || 'M, L, XL, XXL'}`
    );
  }
 ...
 */
export const resolveShirtType = (typeParam: string): string => {
  // If it's already a valid ObjectId, return it
  ...
  if (!typeId) {
    const validTypes = Array.from(shirtTypeCache.keys())
      ...
      .join(', ');
    throw new Error(
      `Invalid shirt type: '${typeParam}'. Valid types are: ${validTypes || 'Casual, Formal, Wedding, Sports, Vintage'}`
    );
  }
 ...
 */
export const getAvailableShirtTypes = (): string[] => {
  return Array.from(shirtTypeCache.keys()).map(
