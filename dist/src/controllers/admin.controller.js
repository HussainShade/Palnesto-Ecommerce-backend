...
export const adminLogin = async (c) => {
    try {
        ...
        const user = await authenticateAdmin(validatedData.email, validatedData.password);
 ...
 */
export const createSeller = async (c) => {
    try {
        const body = await c.req.json();
        const validatedData = createSellerSchema.parse(body);
        // Create seller user
        const seller = await createUser(validatedData.email, validatedData.password, validatedData.name, 'Seller');
        return c.json({
            success: true,
